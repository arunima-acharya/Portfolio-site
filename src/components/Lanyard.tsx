...
export default function Lanyard({
  ...
  // Defer the Canvas until after hydration/StrictMode settles so the WebGL
  // context isn't created then disposed mid double-mount (causes Context Lost).
...
function Band({
  ...
  useSphericalJoint(j3, card, [[0, 0, 0], [0, 2.61, 0]]);
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      // Subtle idle swing — only when not dragged and nearly at rest
      if (!dragged) {
        const t = state.clock.elapsedTime;
        const angVelZ = ang.z;
        // Only nudge when velocity is low so we don't fight user interactions
        if (Math.abs(angVelZ) < 2.0) {
          card.current.applyTorqueImpulse(
            { x: 0, y: 0, z: Math.sin(t * 0.55) * 0.0448 },
            true
          );
        }
      }
  ...
  return (
    <>
      <group position={[0, 4, 0]}>
        ...
        <RigidBody position={[0.25, 0, 0]} ref={j1} {...segmentProps}><BallCollider args={[0.1]} /></RigidBody>
        <RigidBody position={[0.5, 0, 0]} ref={j2} {...segmentProps}><BallCollider args={[0.1]} /></RigidBody>
        <RigidBody position={[0.75, 0, 0]} ref={j3} {...segmentProps}><BallCollider args={[0.1]} /></RigidBody>
        <RigidBody position={[1, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[1.39, 1.96, 0.01]} />
          <group
            scale={3.92}
            position={[0, -2.10, -0.05]}
            onPointerOver={() => hover(true)}
            onPointerOut={() => hover(false)}
            onPointerUp={e => ((e.target as HTMLElement | null)?.releasePointerCapture(e.pointerId), drag(false))}
            onPointerDown={e => ((e.target as HTMLElement | null)?.setPointerCapture(e.pointerId), drag(new THREE.Vector3().copy(e.point).sub(vec.copy(card.current.translation()))))}
          >
      ...
      <mesh ref={band}>
        <meshLineGeometry />
        <meshLineMaterial color="white" depthTest={false} resolution={isMobile ? [1000, 2000] : [1000, 1000]} useMap map={texture} repeat={[-2, 1]} lineWidth={lanyardWidth} />
      </mesh>
    </>
  );
}
