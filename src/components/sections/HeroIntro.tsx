...
class ErrBound extends Component<{ children: ReactNode }, { err: string | null }> {
  ...
  render() {
    if (this.state.err) return <div style={{ color: '#e8510a', padding: "var(--spacing-16)", fontSize: 12 }}>Error: {this.state.err}</div>;
  ...
  return (
    <section className="relative flex" style={{ height: "100vh", width: "100vw", marginLeft: "calc(-1 * var(--gutter))", marginRight: "calc(-1 * var(--gutter))", paddingTop: "10%" }}>
      ...
      <div className="flex flex-col justify-between z-10" style={{ width: "50%", paddingLeft: "10%", paddingRight: "4%", paddingBottom: "calc(8% + 16px)" }}>
        ...
        {/* Heading */}
        <div style={{ marginBottom: "var(--spacing-28)" }}>
        ...
        <div style={{ display: "flex", gap: "var(--spacing-16)", height: "48.6vh" }}>

          {/* Column 1 */}
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "var(--spacing-16)" }}>
            {/* Card A: Clients */}
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: "var(--radius-2xl)", padding: "16px 20px", display: "flex", alignItems: "center", height: "20%", flexShrink: 0 }}>
              {[
              ...
              <span style={{ fontSize: "10px", fontWeight: 600, letterSpacing: "0.12em", color: textMuted, marginLeft: "var(--spacing-16)", whiteSpace: "nowrap" }}>40+ PARTNERS</span>
            </div>

            {/* Card C */}
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: "var(--radius-2xl)", padding: "var(--spacing-20)", display: "flex", flexDirection: "column", justifyContent: "space-between", flex: 1 }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "var(--spacing-16)" }}>
            ...
            {/* Card D: Availability */}
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: "var(--radius-2xl)", padding: "16px 20px", display: "flex", alignItems: "center", height: "20%", flexShrink: 0 }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "var(--spacing-8)", background: isLight ? "rgba(0,0,0,0.04)" : "rgba(255,255,255,0.06)", borderRadius: "100px", padding: "6px 14px" }}>
