  ...
  return (
    <div className={`sm-scope z-50 ${isFixed ? 'fixed top-0 left-0 w-screen h-screen overflow-hidden pointer-events-none' : 'w-full h-full'}`}>
      ...
      >
        ...
        <header className="staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between pointer-events-none z-20" style={{ padding: '1.5em 2em' }}>
          {/* Logo / Brand */}
          <div className="sm-logo flex items-center select-none pointer-events-auto" aria-label="Logo">
            {logoUrl ? (
              <img src={logoUrl} alt="Logo" className="sm-logo-img block h-8 w-auto object-contain" draggable={false} />
            ) : (
              <span style={{ fontSize: 15, fontWeight: 700, color: menuButtonColor, letterSpacing: '-0.02em', fontFamily: 'var(--font-inter), sans-serif', display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#e8510a', display: 'inline-block' }} />
                Arunima
              </span>
            )}
          </div>
          ...
          <button
            ...
            onClick={toggleMenu}
            type="button"
            style={{ fontSize: 14, fontWeight: 400, letterSpacing: '-0.01em', fontFamily: 'var(--font-manrope), sans-serif' }}
          >
        ...
        <aside
          ...
          style={{ padding: '5.5em 2.5em 2.5em 2.5em', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}
          aria-hidden={!open}
        >
      ...
      <style>{`
        ...
        .sm-scope .sm-toggle:focus-visible { outline: 2px solid #ffffff88; outline-offset: 4px; border-radius: 4px; }
        ...
        .sm-scope .staggered-menu-panel { position: absolute; top: 0; right: 0; width: clamp(280px, 40vw, 460px); height: 100%; background: #fff; display: flex; flex-direction: column; overflow-y: auto; z-index: 10; }
        ...
        .sm-scope .sm-panel-item { position: relative; color: #0a0a0a; font-weight: 700; font-size: clamp(2.8rem, 5vw, 4rem); cursor: pointer; line-height: 1; letter-spacing: -2px; text-transform: uppercase; display: inline-block; text-decoration: none; padding-right: 1.4em; transition: color 0.2s ease; }
        .sm-scope .sm-panel-itemLabel { display: inline-block; will-change: transform; transform-origin: 50% 100%; }
        .sm-scope .sm-panel-item:hover { color: var(--sm-accent, #e8510a); }
        .sm-scope .sm-panel-list[data-numbering] { counter-reset: smItem; }
        .sm-scope .sm-panel-list[data-numbering] .sm-panel-item::after { counter-increment: smItem; content: counter(smItem, decimal-leading-zero); position: absolute; top: 0.1em; right: 3.2em; font-size: 16px; font-weight: 400; color: var(--sm-accent, #e8510a); letter-spacing: 0; pointer-events: none; user-select: none; opacity: var(--sm-num-opacity, 0); }
        .sm-scope .sm-socials { margin-top: auto; padding-top: 2rem; display: flex; flex-direction: column; gap: 0.75rem; }
        .sm-scope .sm-socials-title { margin: 0; font-size: 0.85rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; }
        .sm-scope .sm-socials-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: row; align-items: center; gap: 1rem; flex-wrap: wrap; }
        .sm-scope .sm-socials-link { font-size: 1rem; font-weight: 500; color: #333; text-decoration: none; transition: color 0.2s ease; }
        .sm-scope .sm-socials-link:hover { color: var(--sm-accent, #e8510a); }
